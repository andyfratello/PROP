...
public class TestFull
{
    ...
    public void TestConstructora()
    {
        ...
        assertEquals(f1.GetNumCol(),10);
        assertEquals(f1.GetNumFil(),15);
        f1.PrintFull();
    ...
    {
        _full.AfegirCol(_full.GetNumCol());
    ...
    {
        System.out.println("Test Ordenar Columnes de forma Ascendent");

        _full.ModificaCela(0,0,"aaa A1");
        _full.ModificaCela(0,1,"zxy A5");
        _full.ModificaCela(0,2,"122 A2");
        _full.ModificaCela(0,4,"123 A3");
        _full.ModificaCela(0,3,"abc A4");
        _full.ModificaCela(0,6,"zzz A6");

        _full.ModificaCela(1,0,"aaa B1");
        _full.ModificaCela(1,1,"zxy B5");
        _full.ModificaCela(1,2,"abc B4");
        _full.ModificaCela(1,3,"123 B3");
        _full.ModificaCela(1,4,"122 B2");
        _full.ModificaCela(1,6,"zzz B5");

        System.out.println("Full abans d'ordenar la columna 'A,B,C'");
        _full.PrintFull();

        Cela c = _full.GetCela(0,2);
        Cela c2 = _full.GetCela(1,2);
        _full.OrdenarBloc(new Pair<>(2,1), new Pair<>(0,5), true);
        assertSame(c, _full.GetCela(0,1));
        assertSame(c2, _full.GetCela(1,3));

        System.out.println("Full despres d'ordenar la columna 'A',B,C");
    }
     ...
     * l’string 2B" i per a cada cel·la amb un caràcter 'B', es comprova que la cel·la retornada per CercarValor() i
     * el GetCela() de les posicions retornades, són la mateixa Cela amb un assertSame()
     */
    @Test
    public void TestCercaValor()
    {
        Set<Pair<Pair<Integer,Integer>,String>> tuples = _full.CercarValor("A1");
        for (Pair<Pair<Integer,Integer>,String> t : tuples) { assertEquals(t.second(),_full.GetCela(t.first().first(),t.first().second()).GetValorUsuari()); }
